...
import { logout } from '../../services/authService'; // <--- Подключаем серви

const AdminRightSidebarMenu = () => {
            ...
            {adminRoutes.map((section) => (
              <div key={section.id} className="accordion-item bg-dark border-0">
                {section.children && Array.isArray(section.children) ? (
                  ...
                  <Link
                    to={section.path}
                    className="nav-link text-white ps-3 py-2"
                  >
                    {section.label}
                  </Link>
                )}
...
export default AdminRightSidebarMenu;
